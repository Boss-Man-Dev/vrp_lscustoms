...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		LSCMenu:addPurchase("Repair Vehicle",round(10000+150*damage,0), "Full body repair and engine service.")
		...
		LSCMenu.categories:addSubMenu("BODY", "Body", "All Body Modifications.", true)
		AddMod(0,LSCMenu.categories.Body,"SPOILER", "Spoiler", "Increase downforce.",true)
		AddMod(3,LSCMenu.categories.Body,"SKIRTS", "Skirts", "Enhance your vehicle's look with custom side skirts.",true)
		AddMod(4,LSCMenu.categories.Body,"EXHAUST", "Exhausts", "Customized sports exhausts.",true)
		AddMod(6,LSCMenu.categories.Body,"GRILLE", "Grille", "Improved engine cooling.",true)
		AddMod(7,LSCMenu.categories.Body,"HOOD", "Hood", "Enhance car engine cooling.",true)
		AddMod(8,LSCMenu.categories.Body,"FENDERS", "Fenders", "Enhance body paneling with custom fenders.",true)
		AddMod(10,LSCMenu.categories.Body,"ROOF", "Roof", "Lower your center of gravity with lightweight roof panels.",true)
		AddMod(14,LSCMenu.categories.Body,"HORN", "Horn", "Custom air horns.",true)
		LSCMenu.categories.Body:addSubMenu("PLATES", "Plates","Decorative identification.", true)
		LSCMenu.categories.Body.Plates:addSubMenu("LICENSE", "License", "",true)
		for n, mod in pairs(LSC_Config.prices.plates) do
			local btn = LSCMenu.categories.Body.Plates.License:addPurchase(mod.name,mod.price)btn.plateindex = mod.plateindex
		end
		AddMod(25,LSCMenu.categories.Body.Plates, "Plate holder", "Plate holder", "",true) 
		AddMod(26,LSCMenu.categories.Body.Plates, "Vanity plates", "Vanity plates", "",true) 
		AddMod(38,LSCMenu.categories.Body,"HYDRAULICS","Hydraulics","",true)
		AddMod(39,LSCMenu.categories.Body, "ENGINE BLOCK", "Engine Block", "Custom engine block casings.",true)
		AddMod(40,LSCMenu.categories.Body, "CAM COVER", "Cam Cover", "Optional cam covers.",true)
		AddMod(41,LSCMenu.categories.Body, "STRUT BRACE", "Strut Brace", "A selection of support struts.",true)
		AddMod(48,LSCMenu.categories.Body,"Liveries", "Liveries", "A selection of decals for your vehicle.",true)
		m = LSCMenu.categories.Body:addSubMenu("WINDOWS", "Windows", "A selection of tinted windows.",true)	
		...
		LSCMenu.categories:addSubMenu("PERFORMANCE", "Performance", "All your upgrading needs.", true)
		AddMod(12,LSCMenu.categories.Performance,"BRAKES", "Brakes", "Increase stopping power and eliminate brake fade.",true)
		AddMod(13,LSCMenu.categories.Performance,"TRANSMISSION", "Transmission", "Improved acceleration with close ratio transmission.",true)
		AddMod(15,LSCMenu.categories.Performance,"SUSPENSION","Suspension","Upgrade to a sports oriented suspension setup.",true)
		AddMod(16,LSCMenu.categories.Performance,"ARMOR","Armor","Protect your car's occupants with military spec composite body panels.",true)
		AddMod(18,LSCMenu.categories.Performance, "TURBO", "Turbo", "Reduced lag turbocharger.",false)
		AddMod(11, LSCMenu.categories.Performance,"ENGINE TUNES", "Engine Tunes", "Increases horsepower.",true)
		...
		if bumper then
			LSCMenu.categories.Body:addSubMenu("BUMPERS", "Bumpers", "Custom front and rear bumpers.",true)
			if fbumper then
				AddMod(1,LSCMenu.categories.Body.Bumpers,"FRONT BUMPERS", "Front bumpers", "Custom front bumpers.",true)
			end
			if rbumper then
				AddMod(2,LSCMenu.categories.Body.Bumpers,"REAR BUMPERS", "Rear bumpers", "Custom rear bumpers.",true)
			end
		end
					...
					for i,pos in ipairs(garages) do
						...
						if LSC_Config.oldenter then			
							if GetDistanceBetweenCoords(outside.x,outside.y,outside.z,GetEntityCoords(ped)) <= f(5) then
								--checks if our vehicle is on black list
		...
		vRP.EXT.lscustoms.remote._lscustom_doPayment(price)
			...
			if waitingPayment == 2 then
				if LSCMenu.currentmenu == "main" then
					...
					LSCMenu:showNotification("Purchased Complete.")
				end
				waitingPayment = 0
				return true
			elseif waitingPayment == 3 then
				LSCMenu:showNotification("~r~Not enought Funds.")
				waitingPayment = 0
				return false
			end
	else
